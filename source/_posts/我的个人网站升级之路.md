...
```
    ...
    ![](http://storage.jingwangli.com/img/20191220220317.png)
...
出现上述错误的原因是，我删除了子文件夹中的`.git`文件夹(可参考[该文章](https://jingwangli.com/posts/github.html#Changes-not-staged-for-commit))，而父文件夹中的`.git`文件夹中还记录着其相关信息，将父文件夹中的`.git`文件夹也删去然后重新执行`git init`即可。
...
2. 将插件压缩后上传，依赖用url表示，如`"hexo": "https://storage.jingwangli.com/modified_node_modules/hexo.tar"`。
